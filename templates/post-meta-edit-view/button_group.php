        ...
        <div class="acf-button-group">
			...
			if ( !empty($field_data['choices']) ) {
				...
				foreach ($field_data['choices'] as $key => $value) {
					...
					echo '<label class="acf-radio-choices'.$selected.'"><input id="acf-'.esc_attr($field_key).'-Option-'.esc_attr($number).'" type="radio" name="acf['.esc_attr($field_key).']"  value="'.esc_attr($key).'" '.esc_attr($checked).'>'.esc_html__($value).'</label>';
				}
			}
			?>
        </div>
